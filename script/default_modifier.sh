#!/usr/bin/env bash
# Generate rectilinear DEF variants with rectangular cutouts (holes).
# Full-featured version with robust geometry, overlap detection, and safe corrections.

PY_SCRIPT="modify_def.py"
INPUT_DEF="input_file.def"
OUT_DIR="out_defs"     # Default output directory
...
MIN_RECTS=1
MAX_RECTS=6
MIN_DEPTH=100000       # Minimum depth of cutouts
MAX_DEPTH=500000       # Maximum depth of cutouts
...
CORNER_MARGIN_PCT=30   # Keep cutouts away from corners by this % of edge length

# Desired/central aspect ratio for cutouts (width/height). Default 1.0 == 1:1.
# The sampling code uses a truncated normal centered at this value. The truncation
# bounds are computed as [ASPECT_CENTER/2, ASPECT_CENTER*2] so the default range
# is 0.5..2.0 (i.e. 1:2 .. 2:1). You can tweak ASPECT_CENTER to bias shapes.
ASPECT_CENTER=${ASPECT_CENTER:-1.3}
...
# ------------------------------------------------
# Overlap check: returns 0 if two rects overlap
# Rectangles are given as (x1,y1)-(x2,y2) pairs.
# Touching edges are allowed (no buffer).
# ------------------------------------------------
cutout_conflict() {
  ...
  # Normalize rectangles so x1<=x2, y1<=y2 (purely for checking)
  ...
  # Expand by buffer if you want to forbid touching edges
...
for i in $(seq 1 $NUM_VARIANTS); do
  ...
  coords=()   # flat array [edge_x edge_y int_x int_y ...]
  ...
  for j in $(seq 1 $rects); do
    ...
    while [ $tries -lt $MAX_TRIES_PER_RECT ]; do
      ...
      # -----------------------------
      # Placement type among 8 cases:
      # 0=left, 1=right, 2=bottom, 3=top,
      # 4=bottom-left, 5=bottom-right, 6=top-right, 7=top-left
      # Also includes chance to align to an existing cutout.
      # -----------------------------
  ...
  log_debug "    Selected placement type (from rng_helper): $typ"
      ...
      if [ $existing_count -gt 0 ] && [ $adj_chance -lt 30 ]; then
        ...
        # Existing rect span and center (normalize for span/center only)
      ...
      # --------------------------------------------
      # For each placement type, compute edge/internal.
      # Then clamp & auto-correct later.
      # The original complex limits (span limits, margins) are preserved.
      # --------------------------------------------
      case $typ in
        0)  # LEFT EDGE: vertical span varies; rectangle grows rightward
            ...
            if [ $adj_flag -eq 1 ]; then
              ...
              # Sample aspect ratio r from a truncated normal centered at 1.0
        ...
        4)  # BOTTOM-LEFT CORNER: starts at (x0,y0), extends right/up
            ...
            span_x=$(( MIN_DEPTH + ((s*11 + tries*7) % (max_allowed_x - MIN_DEPTH + 1)) ))
            ...
            if [ $max_y -lt $min_y ]; then
              span_y=$min_y
              [ $span_y -gt $max_y ] && span_y=$max_y
            else
              span_y=$(( min_y + ((s*13 + tries*5) % (max_y - min_y + 1)) ))
            fi
        ...
        5)  # BOTTOM-RIGHT CORNER: starts at (x1,y0), extends left/up
            ...
            [ $max_allowed_x -gt $orig_width ] && max_allowed_x=$orig_width
            [ $max_allowed_x -lt $MIN_DEPTH ] && max_allowed_x=$MIN_DEPTH
            span_x=$(( MIN_DEPTH + ((s*17 + tries*3) % (max_allowed_x - MIN_DEPTH + 1)) ))
            ...
            if [ $max_y -lt $min_y ]; then
              ...
              span_y=$(( min_y + ((s*19 + tries*11) % (max_y - min_y + 1)) ))
            fi
        ...
        6)  # TOP-RIGHT CORNER: starts at (x1,y1), extends left/down
            ...
            [ $max_allowed_x -gt $orig_width ] && max_allowed_x=$orig_width
            [ $max_allowed_x -lt $MIN_DEPTH ] && max_allowed_x=$MIN_DEPTH
            span_x=$(( MIN_DEPTH + ((s*23 + tries*13) % (max_allowed_x - MIN_DEPTH + 1)) ))
            ...
            if [ $max_y -lt $min_y ]; then
              ...
              span_y=$(( min_y + ((s*29 + tries*17) % (max_y - min_y + 1)) ))
            fi
        ...
        7)  # TOP-LEFT CORNER: starts at (x0,y1), extends right/down
            ...
            [ $max_allowed_x -gt $orig_width ] && max_allowed_x=$orig_width
            [ $max_allowed_x -lt $MIN_DEPTH ] && max_allowed_x=$MIN_DEPTH
            span_x=$(( MIN_DEPTH + ((s*31 + tries*19) % (max_allowed_x - MIN_DEPTH + 1)) ))
            ...
            if [ $max_y -lt $min_y ]; then
              ...
              span_y=$(( min_y + ((s*37 + tries*23) % (max_y - min_y + 1)) ))
            fi
      ...
      # -----------------------------
      # Normalize semantics:
      # Ensure: edge point is on DIEAREA, internal point strictly inside.
      # Includes final auto-fix to avoid both-on-edge or both-inside.
      # -----------------------------

      # If our "edge" is not on boundary but "internal" is, swap them.
      ...
      # Clamp internal strictly inside (never negative; honor DIEAREA start)
      ...
      # If degenerate (edge==internal), push internal toward die center
      ...
      # Final validation & auto-fix before storing into coords[]
      ...
      # Case 1: both on boundary -> push internal inward safely
      ...
      # Case 2: neither on boundary -> move edge to nearest boundary
      ...
      # Final strict inside clamp for internal point
      [ $int_x -le $x0 ] && int_x=$((x0 + 2))
      [ $int_x -ge $x1 ] && int_x=$((x1 - 2))
      [ $int_y -le $y0 ] && int_y=$((y0 + 2))
      [ $int_y -ge $y1 ] && int_y=$((y1 - 2))
  ...
  # If none placed, fallback to a minimal rectangle
  num_rects=$(( ${#coords[@]} / 4 ))
  ...
  # ---------------------------------------------
  # Snap tiny gaps (<=2% of die size) to remove seams
  # Only if we have at least two rectangles
  # ---------------------------------------------
  if [ $num_rects -gt 1 ]; then
    thr_x=$(( orig_width / 50 ));  [ $thr_x -lt 1 ] && thr_x=1
    thr_y=$(( orig_height / 50 )); [ $thr_y -lt 1 ] && thr_y=1

    # Build min/max arrays (for adjacency checks only; do not change coords semantics)
    minx=(); miny=(); maxx=(); maxy=()
    ...
    # Helper: find which index in the pair is the edge point (0 or 1)
    edge_idx_of_pair() {
    ...
    for ((ri=0; ri<num_rects; ri++)); do
      for ((rj=ri+1; rj<num_rects; rj++)); do
        # ----- Horizontal snapping (ri left, rj right) -----
        ...
        # ----- Vertical snapping (ri below, rj above) -----
  ...
  if python3 "$PY_SCRIPT" -i "$INPUT_DEF" -o "$outf" -r "$num_rects" -c "${coords[@]}" 2>"$err_log"; then
    success=$((success+1))
    # Remove empty error log file if succeeded
